...
import com.mycompany.action.ResponseAction;
		...
		try {
			...
			for (int i = 0; i < trends.size(); i++) {
				...
				for (int hash_index = 0; hash_index < hash_trends.size(); hash_index++)
				{
					...
					JSONObject hashObject = (JSONObject) hash_trends.get(hash_index);
					...
					if (!hashObject.get("text").toString().equals("")
						...
						hashTagVo.setId(jsObject.get("id").toString());
						 ...
						 System.out.println("hashTagList is >>>>>>>>>>>>>>>>>>>>> "+hashObject.get("text").toString());//hashTagList.get(hash_index));
						 System.out.println("hashTagList ID is is >>>>>>>>>>>>>>>>>>>>> "+jsObject.get("id").toString());
						 
						 ResponseAction res=new ResponseAction();
						 res.execute(jsObject.get("id").toString());
