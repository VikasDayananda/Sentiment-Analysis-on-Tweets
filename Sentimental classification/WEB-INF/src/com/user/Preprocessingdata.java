				...
				System.out.println("===============preprocessdata=========="+preprocessdata);
				 ...
				 session.removeAttribute( "waitPage" );  				
												...
												HashMap<Integer, ArrayList<String>> map_tclaim=new HashMap<Integer, ArrayList<String>>();
										     	map_tclaim=AdminDAO.gettweets(preprocessdata);
									     	...
									     	if(map_tclaim.size()>0)
									           ...
									           for(Map.Entry m4:map_tclaim.entrySet())  
									           {
									        	  // System.out.println("entry set is>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+map_tclaim.entrySet());
											         ...
											         System.out.println("Column 3 is>>>>>>>>>>>>>>>>>>>>"+part3);
											         String part4=a[3];
											         System.out.println("Column 4 is>>>>>>>>>>>>>>>>>>>>"+part4);
											        // String part5=a[4];
											        // System.out.println("Column 5 is>>>>>>>>>>>>>>>>>>>>"+part5);	*/	         
											       /*   part5= Temp.removehttplink(part5);*/
											          
											        String  part5= Temp.removeUrl(part4);
											         System.out.println("String Part 5 is>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+part5);		         
											         data=Preprocessing1.filter(part5);
											         ...
											         data.add(str);
														...
														s=s.replace("[", "");
														s=s.replace("]", "");
													 	flag1= AdminDAO.storefiltereddata(part1, part2,part3,s);
